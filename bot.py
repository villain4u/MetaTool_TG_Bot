...
from helpers.m3u8w import getm3u8w
...
for i in CHAT_IDS:
    if len(i) != 0 and i.isnumeric() == True:
        bot.send_message(int(i), "`*Hey ! VTube Manager Bot Is Started*`", parse_mode=telegram.ParseMode.MARKDOWN)
    else:
        pass
    ...
    def wrapped(update, *args, **kwargs):
        ...
        if str(chat_id).startswith("-100"):
            if (grprestricted_mode) and (str(chat_id) not in GRP_LIST):
                ...
                bot.send_message(update.chat.id, "*Error :\t\t*This Group is not Authorized to access the bot.\n\n*Use This Bot In Official VTube Group Only*\n\n[Join Official VTube Channel Now](https://t.me/VTube_Movies) !!", parse_mode='Markdown', disable_web_page_preview=True)
                return
...
@bot.message_handler(commands=['m3u8w'])
@restricted
def m3u8w(m):
    getm3u8w(m)
