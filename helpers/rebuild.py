...
def rebuildmes(m):
    ...
    try:
        ...
        if res["code"] == 200 and res["success"] == True:
            bot.send_message(m.chat.id, text="`VTube Movies ReBuild Successfully !!`", parse_mode=telegram.ParseMode.MARKDOWN)
        else:
        ...
        bot.send_message(m.chat.id, text="`VTube Server Not Accessible !!`", parse_mode=telegram.ParseMode.MARKDOWN)